...
# Word kütüphaneleri ve renk ayarları
...
def word_bas(df_bildiriler, df_ozel, df_mod, tip):
    ...
    if not df_mod.empty:
        for _, r in df_mod.iterrows():
            ...
            for sess in mod_atamalari.keys():
                if (m_gun_saat == '-' or m_gun_saat in sess[0]) and (m_salon == '-' or m_salon == sess[1]):
...
st.markdown("Verileri doğrudan Google Sheets üzerinden okur. Tüm değişiklikleri oradan yapıp Nihai Excel ve Word çıktılarınızı indirebilirsiniz.")
    ...
    with st.spinner('☁️ Veriler buluttan indiriliyor ve işleniyor. Lütfen bekleyin...'):
        try:
            ...
            with col1:
                ...
                st.download_button(label="📥 EXCEL İNDİR", data=excel_yz, file_name="Nihai_Program_Yuzyuze.xlsx", use_container_width=True)
                st.download_button(label="📄 WORD İNDİR", data=word_yz, file_name="Nihai_Program_Yuzyuze.docx", use_container_width=True)
            with col2:
                st.info("🌐 ONLINE PROGRAMI")
                st.download_button(label="📥 EXCEL İNDİR", data=excel_on, file_name="Nihai_Program_Online.xlsx", use_container_width=True)
                st.download_button(label="📄 WORD İNDİR", data=word_on, file_name="Nihai_Program_Online.docx", use_container_width=True)
